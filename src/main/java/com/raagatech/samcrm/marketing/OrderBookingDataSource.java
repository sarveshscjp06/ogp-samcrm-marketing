 ...
 */
@Service
public class OrderBookingDataSource extends DatabaseConnection implements OrderBookingInterface {
    ...
    public void createOrder(OrderDataBean orderDataBean) throws Exception {
        ...
        try {
            ...
            if (key == 0) {
                ...
                key = generateNextPrimaryKey("samcrm_obm_orders", "order_id");
        ...
        try {
            ...
            for (OrderedItemBean itemBean : items) {
                ...
                if (itemBean.getItem_id() > 0) {
                    ...
                    if (result.next()) {
                        int key = generateNextPrimaryKey("samcrm_obm_items", "item_id");
...
//    }
    private int createAddress(long customerId, Connection connection) throws Exception {
        ...
        try {
            ...
            try ( ResultSet result = statement.executeQuery(querycustomerAddress)) {
                if (result.first()) {
                    key = generateNextPrimaryKey("samcrm_obm_address", "address_id");
    ...
    public String completeTransaction(PaymentBean paymentBean) throws Exception {
        ...
        try {
            ...
            int key = generateNextPrimaryKey("samcrm_obm_payment", "transaction_id");
            connection = createConnection();
